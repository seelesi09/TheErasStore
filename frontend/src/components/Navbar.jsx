...
function Navbar({ user, currentView, setCurrentView, handleLogout, onOpenLogin }) {
    ...
    return (
        <>
            ...
            <nav className="w-full bg-black text-white sticky top-0 z-50 border-b border-white/5 font-folklore">
    <div className="max-w-7xl mx-auto w-full px-4 md:px-16 py-4 flex items-center justify-between gap-8">
    {/* Logo */}
    <div
        onClick={() => handleNavClick('pembeli')}
        className="text-lg md:text-xl font-folklore tracking-tight cursor-pointer select-none flex-shrink-0 whitespace-nowrap"
    >
        The Eras Store
    </div>

    {/* Desktop Navigation - centered dengan gap teratur */}
    <div className="hidden md:flex items-center gap-8 text-sm font-medium text-white/60 flex-1 justify-center">
        {navItems.map((item) => (
            <button
                key={item.view}
    ...
    <div className="flex items-center gap-6 flex-shrink-0">
        ...
        {user ? (
            <button
                ...
                className="bg-white hover:bg-white/90 text-black text-xs md:text-sm font-bold px-4 md:px-5 py-2 rounded-full transition-all active:scale-95 cursor-pointer shadow-sm whitespace-nowrap"
            >
                Logout
            </button>
        ) : (
