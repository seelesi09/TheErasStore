...
function OrderHistory({ user, setCurrentView }) {
                            ...
                            <div key={order.Order_ID} className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden">
                                <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex flex-wrap justify-between items-center gap-4 text-xs font-folklore">
                                    <div>
                                        ...
                                        <p className="font-bold text-slate-700">{new Date(order.Tanggal_Order).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                    </div>
                                    <div>
                                        <p className="text-slate-400">TOTAL PRICE</p>
                                        <p className="font-bold text-slate-900 text-sm">Rp {Number(order.Total_Harga).toLocaleString('id-ID')}</p>
                                    </div>
                                    <div>
                                    ...
                                    {order.Items.map((item, idx) => (
                                        <div key={idx} className="flex gap-4 items-center py-4 first:pt-0 last:pb-0">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-xs font-bold text-slate-600 font-folklore mt-1">Rp {Number(item.Harga_Beli).toLocaleString('id-ID')}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
